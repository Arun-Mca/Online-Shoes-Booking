...
public class ExportPdf {
	...
	public static ByteArrayInputStream productReport(ArrayList<MensProducts> mensShoesList,float amount,String userName) {
      ...
      //To display time 
		...
		try {
			...
			for (MensProducts mensShoesList1 : mensShoesList) {

				PdfPCell cell;
				cell = new PdfPCell(new Phrase(String.valueOf(mensShoesList1.getId())));
				cell.setHorizontalAlignment(Element.ALIGN_CENTER);
				table.addCell(cell);

				cell = new PdfPCell(new Phrase(mensShoesList1.getProductName()));
				cell.setHorizontalAlignment(Element.ALIGN_CENTER);
				table.addCell(cell);

				cell = new PdfPCell(new Phrase(mensShoesList1.getPrice()));
				cell.setHorizontalAlignment(Element.ALIGN_CENTER);
				table.addCell(cell);
